...
Listens on :7880, accepts POST /tts with {text, voice}, returns audio/mpeg.
Handles Kokoro's known NaN-output bug with auto-recovery (reload pipeline + retry).
"""
...
# These paths come from the kokoro-tts-cpu base image. Adjust if your base
# image lays things out differently.
...
def get_pipeline(lang_code):
    ...
    if lang_code not in pipelines:
        ...
        # Warmup: first generation on a fresh pipeline can produce NaN.
        # Run a throwaway pass with a voice matching the pipeline's language
        # so the actual code path is exercised.
        warmup_voices = {
            'a': 'af_alloy',   # American
            'b': 'bf_emma',    # British
            'j': 'jf_alpha',   # Japanese
        ...
        try:
            ...
            pass  # voice may not exist for all languages; ignore warmup miss
...
if __name__ == '__main__':
    ...
    # Pre-warm the British pipeline (default voice is bf_emma)
    get_pipeline('b')
